...
def load_dataset(path,train,use_mask = False,mask_path=None):
	...
	if(use_mask):
		...
		idx_ones = np.where(mask==1)[0]
		embeddings = embeddings[idx_ones]
		labels = labels[idx_ones]
		sense_keys = list(np.array(sense_keys)[idx_ones])
		synsets = [sc2ss(sensekey) for sensekey in sense_keys]
...
def main():
	...
	parser.add_argument("--model_type",default=None,type=str,choices=['FC-1layer', 'Projection','Hadamard'],help="type of final model")
	parser.add_argument("--LR",type=float,help='learning rate')
	parser.add_argument("--use_mask",action='store_true',help='whether to use masking')
	parser.add_argument("--mask_path",default=None,type=str,help="The path to mask file")
	...
	embeddings, labels,synsets = load_dataset(args.embeddings_data_dir,args.do_train,args.use_mask,args.mask_path)
	...
	if args.do_train:
		...
		elif(args.model_type=='Hadamard'):
			model = Projection(embeddings.shape[1],graph_embeddings.shape[1],method='hadamard')
		...
		for epoch_no in trange(int(args.num_epochs), desc="Epoch"):
			...
			for iter_i,dev_dataset in enumerate(eval_datasets):
				...
				if(dev_dataset=='semeval2007'):
					semeval_2007_pred = (dev_dataset,dev_prob_values)
			...
			best_accuracy = max(best_accuracy,results_dict['semeval2007'][0])
			if(best_accuracy==results_dict['semeval2007'][0]):
				#result_path = write_results(output_model_file,dev_dataset,dev_prob_values)
				print("saving model..")
