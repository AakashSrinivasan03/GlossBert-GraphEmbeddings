...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id,node_id):
...
def convert_examples_to_features(examples, label_list, max_seq_length,
                                 tokenizer, output_mode,node_2_idx):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        if(example.sense_key in node_2_idx):
            node_id = node_2_idx[example.sense_key]
        else:
            node_id = -1
        ...
        if ex_index < 5:
            ...
            logger.info("node id: %s (id = %d)" % (example.sense_key, node_id))

        features.append(
                InputFeatures(input_ids=input_ids,
                              input_mask=input_mask,
                              segment_ids=segment_ids,
                              label_id=label_id,node_id=node_id))
...
def load_node_embeddings(path):
    ...
    ##########################
    N = 1000
    embed_matrix = np.zeros((1000,128))
    node_2_idx = {i:i for i in range(N)}
    idx_2_node = {i:i for i in range(N)}

    ##########################
    '''d = sp.io.loadmat(path)
    embed_matrix = d['embeddings']
    node_2_idx = d['node_2_idx']
    idx_2_node = d['idx_2_node']'''
    avg = np.mean(embed_matrix,axis=0).reshape(1,-1)
    embed_matrix = np.concatenate([embed_matrix,avg],axis=0)

    return embed_matrix, node_2_idx, idx_2_node
...
def main():
    ...
    parser.add_argument("--node_embedding_file", default=None, type=str, required=False, ######
                        help='''a path to saved node embedding file''')
    ...
    node_embedding_matrix, node_2_idx, idx_2_node = load_node_embeddings(args.node_embedding_file)

    logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                        datefmt = '%m/%d/%Y %H:%M:%S',
                        level = logging.INFO if args.local_rank in [-1, 0] else logging.WARN)

    logger.info("Node embedding loaded..")
    ...
    model = BertForSequenceClassification.from_pretrained(args.bert_model,
              cache_dir=cache_dir,
              num_labels=num_labels,node_embedding_matrix=node_embedding_matrix)
    ...
    if args.do_train:
        train_features = convert_examples_to_features(
            train_examples, label_list, args.max_seq_length, tokenizer, output_mode,node_2_idx)
        ...
        all_node_ids = torch.tensor([f.node_id for f in train_features], dtype=torch.long)
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all_node_ids)
        if args.local_rank == -1:
    ...
    if args.do_eval:
        ...
        all_node_ids = torch.tensor([f.node_id for f in eval_features], dtype=torch.long)
        ...
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_node_ids)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, label_ids,node_ids = batch

                logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=None,node_ids=node_ids)
            ...
            if args.do_eval:
                ...
                with open(os.path.join(args.output_dir, "results_"+str(epoch)+".txt"),"w") as f:
                    for input_ids, input_mask, segment_ids, label_ids, node_ids in tqdm(eval_dataloader, desc="Evaluating"):
